...
@Component
public class LdapUserDetailsAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider
{

	private final static Logger log = LoggerFactory.getLogger(LdapUserDetailsAuthenticationProvider.class);
		...
		throws AuthenticationException
	{
		...
		final LdapAuthenticationDetails details = (LdapAuthenticationDetails) authentication.getDetails();
		try
		{
		...
		catch (LDAPException le)
		{
			...
			log.warn(msg + "; [uid: {}, code: {}, msg: {}]",
				details.getUsername(), le.getResultCode().intValue(), le.getResultCode().getName());
			logService.event("logging.logstash.event.login", "failure", details.getUsername());
	...
	private LdapUserDetails createUserDetails(LDAPConnection connection, LdapAuthenticationDetails details,
		List<GrantedAuthority> groups) throws AuthenticationException
	{
	...
	private List<GrantedAuthority> retrieveGroups(LDAPConnection connection, LdapAuthenticationDetails details)
		throws AuthenticationException
	{
		List<GrantedAuthority> groups = new LinkedList<>();
		ldapService.getGroupsByUser(connection, details.getUsername(), details.getUserDN())
