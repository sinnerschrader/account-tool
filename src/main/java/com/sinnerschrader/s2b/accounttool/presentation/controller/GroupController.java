...
public class GroupController
{
	...
	public ModelAndView listGroups(
		@RequestAttribute(name = WebConstants.ATTR_CONNECTION, required = true) LDAPConnection connection,
		...
		else
		{
			...
			if (success)
			{
				log.info("{} requested access to group {}. A mail was sent to {} admins of group {}", details.getUid(),
					group.getCn(), adminUser.size(), adminGroup.getCn());
				globalMessageFactory.store(request, globalMessageFactory.createInfo("requestAccess.success"));
			}
			else
			{
				log.warn("{} requested access to group {}. The request to group {} failed.", details.getUid(),
					group.getCn(), adminGroup.getCn());
